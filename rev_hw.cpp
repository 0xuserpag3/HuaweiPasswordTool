...
void debug_printbuf(const char *prolog, const char *fmt, void *buf, int sz)
{
  std::printf("%s: ", prolog);
  for (int i = 0; i < sz; ++i) {
...
void rev_HW_AES_AesEncrypt(const char *p_str, uint32_t p_sz, std::string &enc_pass)
{
  ...
  static_assert(sizeof(g_aucEncryptionKey_AES256) <= sizeof(user_aes_key), "check size");
  std::memcpy(user_aes_key, g_aucEncryptionKey_AES256, sizeof(g_aucEncryptionKey_AES256));
...
void rev_HW_AES_WboxDecrypt(const uint8_t *in_key, uint8_t *out_buf)
{
  ...
  arr_table_tmp[1] =
      wapcbctable2[(arr_table[1] >> 24) | 0x700] ^ wapcbctable2[(un_XOR_buf[4]) | 0x400] ^
      wapcbctable2[(((un_XOR_buf[0]) | (un_XOR_buf[1] << 8)) >> 8) | 0x500] ^
      wapcbctable2[((arr_table[2] >> 16) & 0xFF) | 0x600];

  arr_table_tmp[2] =
      wapcbctable2[(arr_table[2] >> 24) | 0xB00] ^ wapcbctable2[(un_XOR_buf[8]) | 0x800] ^
