...
std::string
sanitize_string(const std::string &raw, bool revert_replace = false)
{
...
std::string
encrypt_password(char pass_format, const std::string &pwd)
{
    std::string pass_enc;
...
std::string
decrypt_password(const std::string &pwd)
{
    std::string pass_dec;
        ...
        if (fdec) {
            ...
            while (std::getline(in, pwd_str)) {
                ...
                if (pwd_str.size() < 23 || (pwd_str[1] != '1' && pwd_str[1] != '2')) {
                    std::cout << "[*] Skip. Error format password. " << pwd_str
                              << std::endl;
                    continue;
                }
            ...
            if (pwd_format != '1' && pwd_format != '2') {
                std::cerr << "[-] Format password error '" << pwd_format << '\''
                          << std::endl;
                usage_print();
            }
